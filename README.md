...
- Udostępnianie list: model właściciel + współużytkownicy w danych lokalnych (adres e-mail istniejącego lokalnie użytkownika)
...
W przeglądarce: `http://localhost:4200/` — strona główna, listy, statystyki i profil wymagają zalogowania (przekierowanie na `/login`). Publiczne są tylko `/login` i `/register`.
...
- **Udostępnianie listy na adres e-mail:** działa lokalnie w obrębie tej samej przeglądarki/profilu urządzenia. Adres musi należeć do użytkownika istniejącego w lokalnych danych (`localStorage`), nie jest wysyłana żadna wiadomość e-mail.
...
- List sharing model (owner + shared users in local data; e-mail must belong to an existing local user)
...
Open: `http://localhost:4200/` — the home page, lists, stats, and profile require sign-in (redirect to `/login`). Only `/login` and `/register` are public.
...
- **Share list by e-mail:** works locally within the same browser/device profile. The e-mail must match a user that already exists in local data (`localStorage`), and no real e-mail invitation is sent.
